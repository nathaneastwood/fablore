...
| Lore |||||
| `stories.csv` | `StoryId`, `StoryKey`, `StoryType`, `Title`, `Authors`, `Artists`, `SourceLink`, `PublicationDate`, `ThumbnailImageLink`, `NarratedVideos` | `StoryId` (`ST` + hash of `StoryKey`) | `StoryKey` = path under `src/` (navigation; not used in `story-*.csv` joins). `NarratedVideos` stores JSON array rows with `author` + `url`. | `create_stories_index.py` / `Story` |
