...
def test_load_card_image_lookup_picks_earliest_release_not_file_order(
    ...
    assert lookup["cintari-saber"] == "https://storage.googleapis.com/fabmaster/cardfaces/2020-CRU/CRU079.png"
...
def test_child_image_uses_csv_lookup(src_root: Path, data_dir: Path) -> None:
    lookup = _load_card_image_lookup(data_dir, "weapons")
    image, is_card_art = _child_image(src_root, "weapons/cintari-saber.md", "weapons", lookup)
    assert image == "https://storage.googleapis.com/fabmaster/cardfaces/2020-CRU/CRU079.png"
...
def test_child_image_loose_slug_match(src_root: Path) -> None:
    ...
    lookup = {"talishar-the-lost-prince": "https://storage.googleapis.com/fabmaster/talishar.png"}
    (src_root / "weapons" / "talishar-lost-prince.md").write_text("# Talishar\n")
    image, is_card_art = _child_image(src_root, "weapons/talishar-lost-prince.md", "weapons", lookup)
...
def test_child_image_falls_back_to_page_img_when_no_csv_match(src_root: Path) -> None:
    ...
    image, is_card_art = _child_image(src_root, "weapons/cintari-saber.md", "weapons", {})
...
def test_child_image_plain_has_no_image(src_root: Path) -> None:
    image, is_card_art = _child_image(src_root, "other-characters/the-librarian.md", "plain", {})
    assert image == ""
    assert is_card_art is False


def test_child_image_no_match_no_img_is_empty(src_root: Path) -> None:
    image, is_card_art = _child_image(src_root, "weapons/no-image-weapon.md", "weapons", {})
    assert image == ""
    assert is_card_art is False
...
def test_card_grid_html_renders_image_and_link(src_root: Path, data_dir: Path) -> None:
    ...
    html_out = _card_grid_html("weapons/armed-to-the-teeth.md", src_root, children, "weapons", lookup)
...
def test_card_grid_html_plain_has_no_img_tag(src_root: Path) -> None:
    ...
    html_out = _card_grid_html("other-characters/README.md", src_root, children, "plain", {})
...
def test_card_grid_html_escapes_name() -> None:
    ...
    html_out = _card_grid_html("weapons/armed-to-the-teeth.md", Path("/nonexistent"), children, "plain", {})
